# Research Pilot Project Notes

## Product Positioning

Research Pilot is a local AI research assistant that turns an academic question into an inspectable workflow. It searches papers, checks prior notes, verifies findings against abstracts, formats citations, saves reusable notes, and shows every LangGraph step and MCP tool call in a dashboard.

## Why This Is Not A Normal RAG Chatbot

Most RAG demos hide the retrieval and generation process behind one chat response. Research Pilot exposes the workflow: search, detail lookup, evidence verification, citation formatting, memory, fallback behavior, and confidence labels are all visible.
...
LangGraph makes the control flow explicit. Each node has a clear responsibility, and the final response can show exactly what happened. This makes the workflow easier to evaluate, troubleshoot, and compare across research questions.
...
Search begins with arXiv in normal mode. If arXiv fails or returns too few papers, Semantic Scholar can fill gaps. Successful results are cached briefly. Demo mode uses local fixture papers for a reproducible run and labels them honestly as `demo`.
...
- Demo papers are fixture data for reproducible local runs, not live search results.
- No deployment, auth, or CI/CD is included in the current implementation.
